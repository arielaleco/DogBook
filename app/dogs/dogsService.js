app.factory('dogsService', function ($log, $q, $http) {

    dogsArr = [];
    function Dog(breed , imageUrl) {
        this.breed = breed;
        this.imageUrl=imageUrl;
        this.fetchingNow= "no-fetch";
    }


    GetAllDogsFromAPI();
    function GetAllDogsFromAPI() {

        $http.get("https://dog.ceo/api/breeds/list/all").then(function (response) {
            console.log(response);

            for (var dogName in response.data.message) { 
                console.log(dogName);
                AddDogToArr(dogName);                                
             }            
        },

            function Error() {

                $log.Error;

            });

            
    }

    function AddDogToArr(dogName){
        //var asyncAction = $q.defer();
         
        var oneDogUrl = "https://dog.ceo/api/breed/"+dogName+ "/images/random";
        $http.get(oneDogUrl).then(function (response) {
            var imageUrl = response.data.message;
            dogsArr.push(new Dog(dogName, imageUrl)); 
            dogsArr[dogsArr.length-1].index=dogsArr.length;    
           // asyncAction.resolve(response); 
        }, 
        function (error) {
            dogsArr.push(new Dog(dogName, "data:image/png;base64,"));
            //asyncAction.reject("");            
        })  

        //return asyncAction.promise;
    }

  

    function getOneDogImageFromAPI(dogIndex){
        var asyncAction = $q.defer();
        dogsArr[dogIndex].fetchingNow="fetching-now";   
        var oneDogUrl = "https://dog.ceo/api/breed/"+dogsArr[dogIndex].breed+ "/images/random";
       // dogsArr[dogIndex].fetchingNow="fetching-now";   
        $http.get(oneDogUrl).then(function (response) {    

            dogsArr[dogIndex].fetchingNow="no-fetch";                 
            var imageUrl = response.data.message;                
            dogsArr[dogIndex].imageUrl = imageUrl;
            asyncAction.resolve(dogIndex); 
                                      
        }, 
        function (error) {
            asyncAction.reject("");            
        })  

        return asyncAction.promise;
    }



    function GetAllDogs() {
        // define asynchronous object
        var asyncAction = $q.defer();
        asyncAction.resolve(dogsArr);
        return asyncAction.promise;
    }

    oneBreedDogsArr=[];
    function GetAllDogsFromBreed(breed)
    {
        var asyncAction = $q.defer();
        var oneBreedUrl = "https://dog.ceo/api/breed/"+breed+ "/images";
        
        $http.get(oneBreedUrl).then(function (response) {    
                          
            oneBreedDogsArr = response.data.message;                
            
            asyncAction.resolve(oneBreedDogsArr); 
                                      
        }, 
        function (error) {
            asyncAction.reject("");            
        })  

        return asyncAction.promise;


    }

    var selectedDogImage=""; 
    function setSelectedDogImage(ImageUrl)
    {
        selectedDogImage = ImageUrl;
    }
    
    function getSelectedDogImage()
    {
        return selectedDogImage;
    }

    return {
        GetAllDogs: GetAllDogs,      
        getOneDogImageFromAPI : getOneDogImageFromAPI,
        GetAllDogsFromBreed   : GetAllDogsFromBreed,
        setSelectedDogImage   : setSelectedDogImage,
        getSelectedDogImage   : getSelectedDogImage
        
    }
});